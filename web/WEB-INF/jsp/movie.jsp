<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<!doctype html>
<html lang="ch">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="description" content="左右结构项目，属于大人员的社交工具">
    <meta name="keywords" content="左右结构项目 社交 占座 ">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
    <meta name="format-detection" content="telephone=no">
    <title>猫眼电影后台管理</title>
    <script src="${pageContext.servletContext.contextPath}/static/js/jquery.min.js"></script>
    <script src="${pageContext.servletContext.contextPath}/static/js/bootstrap.min.js"></script>
    <script>
        $(function() {
            $(".meun-item").click(function() {
                $(".meun-item").removeClass("meun-item-active");
                $(this).addClass("meun-item-active");
                var itmeObj = $(".meun-item").find("img");
                itmeObj.each(function() {
                    var items = $(this).attr("src");
                    items = items.replace("_grey.png", ".png");
                    items = items.replace(".png", "_grey.png")
                    $(this).attr("src", items);
                });
                var attrObj = $(this).find("img").attr("src");
                ;
                attrObj = attrObj.replace("_grey.png", ".png");
                $(this).find("img").attr("src", attrObj);
            });
            $("#topAD").click(function() {
                $("#topA").toggleClass(" glyphicon-triangle-right");
                $("#topA").toggleClass(" glyphicon-triangle-bottom");
            });
            $("#topBD").click(function() {
                $("#topB").toggleClass(" glyphicon-triangle-right");
                $("#topB").toggleClass(" glyphicon-triangle-bottom");
            });
            $("#topCD").click(function() {
                $("#topC").toggleClass(" glyphicon-triangle-right");
                $("#topC").toggleClass(" glyphicon-triangle-bottom");
            });
            $(".toggle-btn").click(function() {
                $("#leftMeun").toggleClass("show");
                $("#rightContent").toggleClass("pd0px");
            })
        })
    </script>
    <!--[if lt IE 9]>
    <script src="${pageContext.servletContext.contextPath}/static/js/html5shiv.min.js"></script>
    <script src="${pageContext.servletContext.contextPath}/static/js/respond.min.js"></script>
    <![endif]-->
    <link href="${pageContext.servletContext.contextPath}/static/css/bootstrap.min.css" rel="stylesheet">
    <link href="${pageContext.servletContext.contextPath}/static/js/jquery-3.6.0.js">
    <link rel="stylesheet" type="text/css" href="${pageContext.servletContext.contextPath}/static/css/common.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.servletContext.contextPath}/static/css/slide.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.servletContext.contextPath}/static/css/bootstrap.min.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.servletContext.contextPath}/static/css/flat-ui.min.css" />
    <link rel="stylesheet" type="text/css" href="${pageContext.servletContext.contextPath}/static/css/jquery.nouislider.css">
    <style>
        .a{
            line-height: 20px;
            color: #fff;
            font-size: 12px;
            background: red;
            border: none;
            border-radius: 3px;
        }
        .b{
            background: #2ecc71;
        }
        .clearfix{
            margin-top: 10px;
        }
        .clearfix li{
            display: inline-block;
        }
    </style>
</head>

<body>
<div id="wrap">
    <!-- 左侧菜单栏目块 -->
    <div class="leftMeun" id="leftMeun">
        <div id="logoDiv" style="height: 80px;">
            <a href="//www.maoyan.com" style="    -webkit-text-size-adjust: 100%;
                                                            font-family: Microsoft YaHei,Helvetica,Arial,sans-serif;
                                                            -webkit-font-smoothing: subpixel-antialiased;
                                                            font-size: 2em;
                                                            text-decoration: none;
                                                            background: url(data:image/png;base64,) no-repeat 0;
                                                            background-size: contain;
                                                            overflow: hidden;
                                                            float: left;
                                                            width: 133px;
                                                            height: 80px;
                                                            line-height: 100em;
                                                            margin-top: -20px;" class="logo" data-act="icon-click">猫眼电影</a>
        </div>
        <div class="meun-item"  href="#sour" aria-controls="sour" role="tab" data-toggle="tab" onclick="a()">用户管理</div>
        <div class="meun-item meun-item-active" href="#char" aria-controls="char" role="tab" data-toggle="tab">电影管理</div>
        <div class="meun-item" href="#user" aria-controls="user" role="tab" data-toggle="tab" onclick="b()">订单管理</div>
        <div class="meun-item" href="#scho" aria-controls="scho" role="tab" data-toggle="tab" onclick="c()">场次管理</div>
        <div class="meun-item" href="#scho" aria-controls="scho" role="tab" data-toggle="tab" onclick="d()">回到前台</div>
    </div>
    <!-- 右侧具体内容栏目 -->
    <div id="rightContent">

        <!-- Tab panes -->
        <div class="tab-content">
            <!-- 用户管理模块 -->
            <!-- 电影管理模块 -->
            <div role="tabpanel" class="tab-pane active" id="char">

                <div class="check-div form-inline" style="position: relative;">
                    <div class="check-div" style="position: absolute; left: 0;">
                        <button class="btn btn-yellow btn-xs" data-toggle="modal" data-target="#addChar">添加电影</button>
                    </div>
                    <div style="position: absolute; right: 0;">
                        <input type="text" class=" form-control input-sm " id="search" placeholder="输入电影名搜索">
                        <button class="btn btn-white btn-xs " onclick="a2()">查 询 </button>
                    </div>
                </div>
                <div class="data-div">
                    <div class="row tableHeader">
                        <div class="col-xs-1 ">
                            电影id
                        </div>
                        <div class="col-xs-3">
                            电影中文名
                        </div>
                        <div class="col-xs-2">
                            导演
                        </div>
                        <div class="col-xs-2">
                            时长
                        </div>
                        <div class="col-xs-2">
                            上映时间
                        </div>
                        <div class="col-xs-2">
                            操作
                        </div>
                    </div>

                    <div class="tablebody">
                        <c:forEach items="${movieList}" var="movie">
                            <div class="row">
                                <div class="col-xs-1 ">
                                        ${movie.movieId}
                                </div>
                                <div class="col-xs-3">
                                    </span><span>${movie.movieCnName}</span>
                                </div>
                                <div class="col-xs-2">
                                        ${movie.movieDirector}
                                </div>
                                <div class="col-xs-2">
                                        ${movie.movieDuration}
                                </div>
                                <div class="col-xs-2">
                                        ${movie.movieReleaseDate}
                                </div>
                                <div class="col-xs-2">
<%--                                    <a href="movie?movieId1=${movie.movieId}">--%>
<%--                                        <button class="btn btn-success btn-xs" data-toggle="modal" data-target="#changeChar">修改</button>--%>
<%--                                    </a>--%>
                                 <a href="movie?movieId=${movie.movieId}">
                                     <button class="btn btn-danger btn-xs">删除</button>
                                 </a>
                                </div>
                            </div>
                        </c:forEach>
                    </div>


                </div>
                <!--页码块-->
                <footer class="footer">
                    <ul class="clearfix">
                        <c:if test="${current!=1}">
                            <li><a href="movie?pageNum=1
                                <c:if test="${movieName!=null}">
                                     &movieName=${movieName}
                                 </c:if>
                            ">首页</a></li>


                            <li><a href="movie?pageNum=${current-1}
                                <c:if test="${movieName!=null}">
                                     &movieName=${movieName}
                                 </c:if>
                            ">上一页</a></li>
                        </c:if>


                        <li class="current">${current}/${pages}</li>


                        <c:if test="${current!=pages}">
                            <li><a href="movie?pageNum=${current+1}
                                <c:if test="${movieName!=null}">
                                     &movieName=${movieName}
                                 </c:if>
                            ">下一页</a></li>


                            <li><a href="movie?pageNum=${pages}
                                <c:if test="${movieName!=null}">
                                     &movieName=${movieName}
                                 </c:if>
                            ">末页</a></li>
                        </c:if>
                    </ul>
                </footer>
                <!--增加权限弹出窗口-->
                <div class="modal fade" id="addChar" role="dialog" aria-labelledby="gridSystemModalLabel">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                <h4 class="modal-title">添加电影</h4>
                            </div>
                            <div class="modal-body">
                                <div class="container-fluid" style="width: 450px;">
                                    <form class="form-horizontal">
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">电影名：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="movieName" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">电影英文名：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="movieEnglishName" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">演员：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="actors" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">导演：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="director" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="sLink" class="col-xs-3 control-label">电影描述：</label>
                                            <div class="col-xs-6 ">
                                                <textarea class="form-control input-sm duiqi" id="describe"></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">电影时长：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="duration" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">电影上映时间：</label>
                                            <div class="col-xs-6 ">
                                                <input type="date" class="form-control input-sm duiqi" id="date" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label for="sName" class="col-xs-3 control-label">电影制片地区：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" id="region" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInput1" class="col-xs-3 control-label">电影状态：</label>
                                            <div class="col-xs-8" id="anniu1">
                                                <label class="control-label" for="anniu" >
                                                    <input type="radio" name="leixing1" value="1" checked>上架</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                <label class="control-label" for="meun">
                                                    <input type="radio" name="leixing1" value="0" > 下架</label>
                                            </div>
                                        </div>

                                    </form>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-xs btn-white" data-dismiss="modal">取 消</button>
                                <button type="button" class="btn btn-xs btn-green" onclick="a1()">保 存</button>
                            </div>
                        </div>
                        <!-- /.modal-content -->
                    </div>
                    <!-- /.modal-dialog -->
                </div>
                <!-- /.modal -->

                <!--修改权限弹出窗口-->
                <div class="modal fade" id="changeChar" role="dialog" aria-labelledby="gridSystemModalLabel">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                <h4 class="modal-title" id="gridSystemModalLabel">修改电影</h4>
                            </div>
                            <div class="modal-body">
                                <div class="container-fluid" style="width: 450px;">
                                    <form class="form-horizontal">
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">电影名：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" class="form-control input-sm duiqi" value="${movie.movieCnName}" id="movieName1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">电影英文名：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieFgName}" class="form-control input-sm duiqi" id="movieEnglishName1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label  class="col-xs-3 control-label">演员：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieActor}" class="form-control input-sm duiqi" id="actors1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">导演：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieDirector}" class="form-control input-sm duiqi" id="director1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label">电影描述：</label>
                                            <div class="col-xs-6 ">
                                                <textarea class="form-control input-sm duiqi" value="${movie.movieDetail}" id="describe1"></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">电影时长：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieDuration}" class="form-control input-sm duiqi" id="duration1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">电影上映时间：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieReleaseDate}" class="form-control input-sm duiqi" id="date1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group ">
                                            <label class="col-xs-3 control-label">电影制片地区：</label>
                                            <div class="col-xs-6 ">
                                                <input type="email" value="${movie.movieCountry}" class="form-control input-sm duiqi" id="region1" placeholder="">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label  class="col-xs-3 control-label">电影状态：</label>
                                            <div class="col-xs-8">
                                                <label class="control-label" >
                                                    <input type="radio" name="leixing" value="0" checked>上架</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                                <label class="control-label" >
                                                    <input type="radio" name="leixing" value="1"> 下架</label>
                                            </div>
                                        </div>

                                    </form>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-xs btn-white" data-dismiss="modal">取 消</button>
                                <button type="button" class="btn btn-xs btn-green" onclick="a3()">修 改</button>
                            </div>
                        </div>
                        <!-- /.modal-content -->
                    </div>
                    <!-- /.modal-dialog -->
                </div>
                <!-- /.modal -->

            </div>

            <!--场次管理模块-->

        </div>
    </div>
</div>
<script src="js/jquery.nouislider.js"></script>

<!-- this page specific inline scripts -->
<script>
    //min/max slider
    function huadong(my, unit, def, max) {
        $(my).noUiSlider({
            range: [0, max],
            start: [def],
            handles: 1,
            connect: 'upper',
            slide: function() {
                var val = Math.floor($(this).val());
                $(this).find(".noUi-handle").text(
                    val + unit
                );
                console.log($(this).find(".noUi-handle").parent().parent().html());
            },
            set: function() {
                var val = Math.floor($(this).val());
                $(this).find(".noUi-handle").text(
                    val + unit
                );
            }
        });
        $(my).val(def, true);
    }
    huadong('.slider-minmax1', "分钟", "5", 30);
    huadong('.slider-minmax2', "分钟", "6", 15);
    huadong('.slider-minmax3', "分钟", "10", 60);
    huadong('.slider-minmax4', "次", "2", 10);
    huadong('.slider-minmax5', "天", "3", 7);
    huadong('.slider-minmax6', "天", "8", 10);



    function search(){
        let aa= document.getElementById("userName");
        console("aa"+aa)
        window.location.href= '';
    }
</script>
</body>

</html>
<script>
    function a(){
        location.href = "user";
    }
    function b(){
        location.href = "order";
    }
    function c(){
        location.href = "schedule";
    }
    function d(){
        location.href = "${pageContext.servletContext.contextPath}/mainPage";
    }
    function a2(){
        var search = document.getElementById("search").value;
        location.href="movie?movieName="+search;
    }
    function a1(){
        var movieName = document.getElementById("movieName").value;
        console.log(movieName)
        var movieEnglishName = document.getElementById("movieEnglishName").value;
        console.log(movieEnglishName)
        var actors = document.getElementById("actors").value;
        console.log(actors)
        var director = document.getElementById("director").value;
        console.log(director)
        var describe = document.getElementById("describe").value;
        console.log(describe)
        var duration = document.getElementById("duration").value;
        console.log(duration)
        var date = document.getElementById("date").value;
        console.log(date)
        var region = document.getElementById("region").value;
        console.log(region)
        var leixing1 = document.getElementsByName("leixing1");
        var type;
        for (let i = 0; i < leixing1.length; i++) {
            if (leixing1[i].checked){
                type = leixing1[i].value;
            }
        }
        location.href = "movie?movieName1="+movieName+"&movieEnglishName="+movieEnglishName+"&actors="+actors+
        "&director="+director+"&describe="+describe+"&duration="+duration+"&date="+date+"&region="+region+"&type="+type;

    }
    function a3(){
        var movieName1 = document.getElementById("movieName1").value;
        console.log(movieName1)
        var movieEnglishName1 = document.getElementById("movieEnglishName1").value;
        console.log(movieEnglishName1)
        var actors1 = document.getElementById("actors1").value;
        console.log(actors1)
        var director1 = document.getElementById("director1").value;
        console.log(director1)
        var describe1 = document.getElementById("describe1").value;
        console.log(describe1)
        var duration1 = document.getElementById("duration1").value;
        console.log(duration1)
        var date1 = document.getElementById("date1").value;
        console.log(date1)
        var region1 = document.getElementById("region1").value;
        console.log(region1)
        var leixing = document.getElementsByName("leixing");
        var type;
        for (let i = 0; i < leixing.length; i++) {
            if (leixing[i].checked){
                type = leixing[i].value;
            }
        }
        console.log(type);
    }

</script>